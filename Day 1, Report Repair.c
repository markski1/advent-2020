...
// Remove the PART_TWO define to resolve the first part.
    ...
    for (z = 0; z < 200; z++) {
        ...
        for (y = 0; y < 200; y++) {
            ...
            if ((expenses[z] + expenses[y]) == 2020) {
               printf("The values %d and %d sum to 2020 - ", expenses[z], expenses[y]);
               printf("Multiplied they give %d \n", expenses[z] * expenses[y]);
            }
        	#else
        	for (x = 0; x < 200; x++) {
        		if ((expenses[z] + expenses[y] + expenses[x]) == 2020) {
	               printf("The values %d, %d and %d sum to 2020 - ", expenses[z], expenses[y], expenses[x]);
	               printf("Multiplied they give %d \n", expenses[z] * expenses[y] * expenses[x]);
	            }
			}
        	#endif
        }    
